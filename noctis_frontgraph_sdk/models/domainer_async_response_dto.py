...
class DomainerAsyncResponseDTO(BaseModel):
    """
    Response object for an asynchronous domain similarity search request submission
    """ # noqa: E501
    request_id: StrictStr = Field(description="Unique identifier for tracking the request", alias="requestId")
    domain_name: StrictStr = Field(description="Domain name being searched", alias="domainName")
    query_string: Optional[StrictStr] = Field(default=None, description="Query string submitted for free-text search (if applicable)", alias="queryString")
    message: StrictStr = Field(description="Status message providing information about the request")
    status_url: StrictStr = Field(description="URL to check for results (can be polled to monitor progress and get results)", alias="statusUrl")
    __properties: ClassVar[List[str]] = ["requestId", "domainName", "queryString", "message", "statusUrl"]
