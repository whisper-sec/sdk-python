# JobListResponse

Paginated list of jobs with summary information

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**jobs** | [**List[JobSummary]**](JobSummary.md) | List of job summaries | [optional] 
**total** | **int** | Total number of jobs matching the filter | [optional] 
**limit** | **int** | Maximum number of jobs returned | [optional] 
**offset** | **int** | Number of jobs skipped for pagination | [optional] 
**status** | **str** | Status filter applied (if any) | [optional] 

## Example

```python
from whisper_api_sdk.models.job_list_response import JobListResponse

# TODO update the JSON string below
json = "{}"
# create an instance of JobListResponse from a JSON string
job_list_response_instance = JobListResponse.from_json(json)
# print the JSON string representation of the object
print(JobListResponse.to_json())

# convert the object into a dict
job_list_response_dict = job_list_response_instance.to_dict()
# create an instance of JobListResponse from a dict
job_list_response_from_dict = JobListResponse.from_dict(job_list_response_dict)
```
[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


