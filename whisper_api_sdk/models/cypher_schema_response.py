...
class CypherSchemaResponse(BaseModel):
    """
    ...
    """ # noqa: E501
    node_labels: Optional[List[StrictStr]] = Field(default=None, description="Available node labels in the graph.  Common node types: - `DomainName`: Domain names (e.g., google.com) - `A_ADDRESS`: IPv4 addresses - `AAAA_ADDRESS`: IPv6 addresses - `ASN`: Autonomous System Numbers - `ANNOUNCED_PREFIX_4`: IPv4 prefixes announced via BGP - `ANNOUNCED_PREFIX_6`: IPv6 prefixes announced via BGP - `Organisation`: Organizations owning ASNs - `Country`: Country information - `LIST`: Blocklist/whitelist entries - `LISTCATEGORY`: Categories of lists - `TLD`: Top-level domains - `TLSCA`: TLS Certificate Authorities - `TLSCert`: TLS Certificates ", alias="nodeLabels")
    relationship_types: Optional[List[StrictStr]] = Field(default=None, description="Available relationship types in the graph.  Common relationships: - `hasIP`: Domain → A_ADDRESS (DNS A record) - `hasIPv6`: Domain → AAAA_ADDRESS (DNS AAAA record) - `hasNameserver`: Domain → DomainName (NS record) - `hasMailHost`: Domain → DomainName (MX record) - `hasTLD`: Domain → TLD - `announces`: ASN → ANNOUNCED_PREFIX - `inASN`: IP_PREFIX → ASN - `isListed`: IP/Domain → LIST (blocklist membership) - `hasCA`: TLSCert → TLSCA - `isIssuer`: TLSCA → TLSCert - `isSubjectOf`: DomainName → TLSCert ", alias="relationshipTypes")
    node_properties: Optional[Dict[str, List[StrictStr]]] = Field(default=None, description="Properties available on each node type, keyed by node label", alias="nodeProperties")
    relationship_properties: Optional[Dict[str, List[StrictStr]]] = Field(default=None, description="Properties available on each relationship type, keyed by relationship type", alias="relationshipProperties")
    node_count: Optional[StrictInt] = Field(default=None, description="Total number of nodes in the graph", alias="nodeCount")
    relationship_count: Optional[StrictInt] = Field(default=None, description="Total number of relationships in the graph", alias="relationshipCount")
    __properties: ClassVar[List[str]] = ["nodeLabels", "relationshipTypes", "nodeProperties", "relationshipProperties", "nodeCount", "relationshipCount"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "nodeProperties": obj.get("nodeProperties"),
            "relationshipProperties": obj.get("relationshipProperties"),
            "nodeCount": obj.get("nodeCount"),
            "relationshipCount": obj.get("relationshipCount")
        })
        return _obj
