...
class InfraScanRequest(BaseModel):
    ...
    """ # noqa: E501
    target: StrictStr = Field(description="The target to scan (IP, CIDR, domain, or ASN)")
    target_type: Annotated[str, Field(strict=True)] = Field(description="Type of target being scanned")
    scan_types: Optional[List[StrictStr]] = Field(default=None, description="Types of scans to perform")
    scan_depth: Optional[Annotated[str, Field(strict=True)]] = Field(default='medium', description="Scan depth level")
    timeout: Optional[Annotated[int, Field(le=3600, strict=True, ge=60)]] = Field(default=300, description="Maximum scan duration in seconds")
    options: Optional[ScanOptions] = None
    __properties: ClassVar[List[str]] = ["target", "target_type", "scan_types", "scan_depth", "timeout", "options"]

    @field_validator('target_type')
    def target_type_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^(ip|cidr|domain|asn)$", value):
            raise ValueError(r"must validate the regular expression /^(ip|cidr|domain|asn)$/")
        return value

    @field_validator('target_type')
    def target_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ip', 'cidr', 'domain', 'asn']):
            raise ValueError("must be one of enum values ('ip', 'cidr', 'domain', 'asn')")
        return value

    @field_validator('scan_depth')
    def scan_depth_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^(light|medium|deep)$", value):
            raise ValueError(r"must validate the regular expression /^(light|medium|deep)$/")
        return value

    @field_validator('scan_depth')
    def scan_depth_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['light', 'medium', 'deep']):
            raise ValueError("must be one of enum values ('light', 'medium', 'deep')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "scan_types": obj.get("scan_types"),
            "scan_depth": obj.get("scan_depth") if obj.get("scan_depth") is not None else 'medium',
            "timeout": obj.get("timeout") if obj.get("timeout") is not None else 300,
