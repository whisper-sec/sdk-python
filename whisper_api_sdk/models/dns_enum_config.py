...
class DnsEnumConfig(BaseModel):
    """
    Configuration for DNS enumeration and subdomain discovery
    """ # noqa: E501
    subdomain_enum: Optional[StrictBool] = Field(default=True, description="Enable subdomain enumeration")
    zone_transfer: Optional[StrictBool] = Field(default=False, description="Enable DNS zone transfer attempts")
    reverse_dns: Optional[StrictBool] = Field(default=True, description="Enable reverse DNS lookups")
    wordlist_size: Optional[Annotated[str, Field(strict=True)]] = Field(default='medium', description="Wordlist size for subdomain brute-forcing")
    __properties: ClassVar[List[str]] = ["subdomain_enum", "zone_transfer", "reverse_dns", "wordlist_size"]

    @field_validator('wordlist_size')
    def wordlist_size_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^(small|medium|large)$", value):
            raise ValueError(r"must validate the regular expression /^(small|medium|large)$/")
        return value

    @field_validator('wordlist_size')
    def wordlist_size_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "subdomain_enum": obj.get("subdomain_enum") if obj.get("subdomain_enum") is not None else True,
            "zone_transfer": obj.get("zone_transfer") if obj.get("zone_transfer") is not None else False,
            "reverse_dns": obj.get("reverse_dns") if obj.get("reverse_dns") is not None else True,
            "wordlist_size": obj.get("wordlist_size") if obj.get("wordlist_size") is not None else 'medium'
        })
        return _obj
