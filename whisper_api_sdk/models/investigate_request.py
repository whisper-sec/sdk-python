...
class InvestigateRequest(BaseModel):
    ...
    """ # noqa: E501
    indicator: Annotated[str, Field(min_length=1, strict=True)] = Field(description="The indicator to investigate (IP address, domain name, or file hash)")
    indicator_type: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Type of indicator being investigated", alias="indicatorType")
    depth: Optional[Annotated[str, Field(strict=True)]] = Field(default='standard', description="Investigation depth level. 'quick' (30-60s) for basic checks, 'standard' (60-120s) for typical investigations, 'comprehensive' (120-300s) for deep analysis.")
    context: Optional[InvestigateContext] = Field(default=None, description="Optional context object to guide the investigation. NOT a string - must be an object with fields like hypothesis, relatedIndicators, timeRange.")
    __properties: ClassVar[List[str]] = ["indicator", "indicatorType", "depth", "context"]
    ...
    def depth_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(quick|standard|comprehensive)$", value):
            raise ValueError(r"must validate the regular expression /^(quick|standard|comprehensive)$/")
        return value
    ...
    def depth_validate_enum(cls, value):
        ...
        if value not in set(['quick', 'standard', 'comprehensive']):
            raise ValueError("must be one of enum values ('quick', 'standard', 'comprehensive')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "depth": obj.get("depth") if obj.get("depth") is not None else 'standard',
            "context": InvestigateContext.from_dict(obj["context"]) if obj.get("context") is not None else None
        })
        return _obj
