...
class SearchResponse(BaseModel):
    ...
    """ # noqa: E501
    source: StrictStr = Field(description="The data source that was searched")
    query_type: StrictStr = Field(description="The type of query that was performed", alias="queryType")
    items: List[DomainItem] = Field(description="List of matching domains")
    total: StrictInt = Field(description="Total number of matching records across all pages")
    page_number: StrictInt = Field(description="Current page number (0-indexed)", alias="pageNumber")
    page_size: StrictInt = Field(description="Number of results per page", alias="pageSize")
    total_pages: StrictInt = Field(description="Total number of pages available", alias="totalPages")
    error: Optional[StrictBool] = Field(default=None, description="Error flag, present only when the search failed")
    message: Optional[StrictStr] = Field(default=None, description="Error message, present only when the search failed")
    __properties: ClassVar[List[str]] = ["source", "queryType", "items", "total", "pageNumber", "pageSize", "totalPages", "error", "message"]

    @field_validator('source')
    def source_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['whois', 'domainer', 'geograph']):
            raise ValueError("must be one of enum values ('whois', 'domainer', 'geograph')")
        return value

    @field_validator('query_type')
    def query_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['registration_data', 'domain_search', 'location_search']):
            raise ValueError("must be one of enum values ('registration_data', 'domain_search', 'location_search')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "items": [DomainItem.from_dict(_item) for _item in obj["items"]] if obj.get("items") is not None else None,
            "total": obj.get("total"),
