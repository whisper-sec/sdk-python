...
class MonitorCheckRequest(BaseModel):
    ...
    """ # noqa: E501
    name: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Name of the monitoring check")
    url: Annotated[str, Field(min_length=1, strict=True)] = Field(description="URL to monitor")
    type: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Type of check: api, ssl, or dns")
    ...
    assertions: Optional[MonitorAssertions] = Field(default=None, description="Assertions for the check")
    enabled: Optional[StrictBool] = Field(default=None, description="Whether the check is enabled")
    method: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="HTTP method for API checks")
    ...
    __properties: ClassVar[List[str]] = ["name", "url", "type", "frequency", "locations", "assertions", "enabled", "method", "headers", "body"]
    ...
    def type_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(api|ssl|dns)$", value):
            raise ValueError(r"must validate the regular expression /^(api|ssl|dns)$/")
        return value
    ...
    def method_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(GET|POST|PUT|DELETE|HEAD|OPTIONS)$", value):
            raise ValueError(r"must validate the regular expression /^(GET|POST|PUT|DELETE|HEAD|OPTIONS)$/")
        return value
