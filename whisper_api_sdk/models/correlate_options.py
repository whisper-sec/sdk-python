...
class CorrelateOptions(BaseModel):
    ...
    """ # noqa: E501
    time_window: Optional[StrictStr] = Field(default=None, description="Time window for temporal correlation. Format: number + unit (d=days, w=weeks, m=months). Examples: 7d, 2w, 3m", alias="timeWindow")
    correlation_types: Optional[List[StrictStr]] = Field(default=None, description="Types of correlation to analyze. Possible values: infrastructure, campaign, actor, malware, technique", alias="correlationTypes")
    min_confidence: Optional[Union[Annotated[float, Field(le=1.0, strict=True, ge=0.0)], Annotated[int, Field(le=1, strict=True, ge=0)]]] = Field(default=None, description="Minimum confidence threshold for correlations (0.0 to 1.0). Higher values return fewer but more confident results.", alias="minConfidence")
    include_industry_breakdown: Optional[StrictBool] = Field(default=None, description="Include breakdown of indicator prevalence by industry sector in the response.", alias="includeIndustryBreakdown")
    include_campaign_detection: Optional[StrictBool] = Field(default=None, description="Include campaign detection analysis to identify coordinated threat activity.", alias="includeCampaignDetection")
    include_geographic_spread: Optional[StrictBool] = Field(default=None, description="Include geographic spread analysis showing indicator distribution across regions.", alias="includeGeographicSpread")
    include_attack_patterns: Optional[StrictBool] = Field(default=None, description="Include attack pattern analysis based on MITRE ATT&CK framework.", alias="includeAttackPatterns")
    anonymization_level: Optional[StrictStr] = Field(default=None, description="Level of data anonymization. Values: none, standard, strict", alias="anonymizationLevel")
    __properties: ClassVar[List[str]] = ["timeWindow", "correlationTypes", "minConfidence", "includeIndustryBreakdown", "includeCampaignDetection", "includeGeographicSpread", "includeAttackPatterns", "anonymizationLevel"]

    @field_validator('anonymization_level')
    def anonymization_level_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['none', 'standard', 'strict']):
            raise ValueError("must be one of enum values ('none', 'standard', 'strict')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "includeCampaignDetection": obj.get("includeCampaignDetection"),
            "includeGeographicSpread": obj.get("includeGeographicSpread"),
            "includeAttackPatterns": obj.get("includeAttackPatterns"),
            "anonymizationLevel": obj.get("anonymizationLevel")
        })
        return _obj
