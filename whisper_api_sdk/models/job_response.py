...
class JobResponse(BaseModel):
    ...
    """ # noqa: E501
    job_id: StrictStr = Field(description="Unique identifier for the job. Use this ID to poll for job status and results.", alias="jobId")
    status: StrictStr = Field(description="Current status of the job")
    status_url: StrictStr = Field(description="URL endpoint to poll for job status and results. Typically `/v1/ops/jobs/{jobId}`.", alias="statusUrl")
    message: Optional[StrictStr] = Field(default=None, description="Human-readable message describing the job acceptance or current state")
    __properties: ClassVar[List[str]] = ["jobId", "status", "statusUrl", "message"]
