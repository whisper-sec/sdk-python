...
class InfrastructureMapRequest(BaseModel):
    """
    Configuration for mapping infrastructure relationships and discovering connected assets
    """ # noqa: E501
    start_point: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Starting point for infrastructure mapping (domain or IP address)", alias="startPoint")
    depth: Optional[Annotated[int, Field(le=3, strict=True, ge=1)]] = Field(default=1, description="Depth of relationship mapping (number of hops from starting point). Depth 1: Direct relationships (~30s, 10-50 assets), Depth 2: 2 hops (~2-5 min, 50-500 assets), Depth 3: 3 hops (~10-30 min, 500-5000 assets)")
    __properties: ClassVar[List[str]] = ["startPoint", "depth"]
