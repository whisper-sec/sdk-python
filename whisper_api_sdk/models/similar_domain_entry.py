...
class SimilarDomainEntry(BaseModel):
    """
    A single similar domain entry with its detection technique
    """ # noqa: E501
    domain: StrictStr = Field(description="The similar domain name. For homoglyph/IDN domains, may include punycode in parentheses.")
    technique: StrictStr = Field(description="The similarity detection technique that found this domain")
    __properties: ClassVar[List[str]] = ["domain", "technique"]

    @field_validator('technique')
    def technique_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['TYPO', 'UTFVARS', 'TLD_VARIANTS', 'SOUNDING', 'PREFIX', 'SUFFIX', 'CONTAINS', 'LEVENSHTEIN']):
            raise ValueError("must be one of enum values ('TYPO', 'UTFVARS', 'TLD_VARIANTS', 'SOUNDING', 'PREFIX', 'SUFFIX', 'CONTAINS', 'LEVENSHTEIN')")
        return value
