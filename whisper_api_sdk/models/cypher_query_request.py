...
class CypherQueryRequest(BaseModel):
    ...
    """ # noqa: E501
    query: Annotated[str, Field(min_length=1, strict=True)] = Field(description="The Cypher query to execute. Must be a read-only query (MATCH/RETURN).  Use `$paramName` syntax to reference parameters from the `parameters` map. ")
    parameters: Optional[Dict[str, Any]] = Field(default=None, description="Query parameters as key-value pairs. Use `$key` in the query to reference these.  Supported value types: String, Number, Boolean, List, Map. ")
    limit: Optional[Annotated[int, Field(le=1000, strict=True, ge=1)]] = Field(default=100, description="Maximum number of results to return. Overrides any LIMIT clause in the query.")
    __properties: ClassVar[List[str]] = ["query", "parameters", "limit"]
