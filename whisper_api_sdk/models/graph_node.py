...
class GraphNode(BaseModel):
    ...
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="Unique identifier for the node (typically the domain name, IP address, or ASN)")
    type: Optional[StrictStr] = Field(default=None, description="Type of infrastructure element this node represents")
    label: Optional[StrictStr] = Field(default=None, description="Human-readable label for display (may differ from id)")
    properties: Optional[Dict[str, Any]] = Field(default=None, description="Additional properties for node styling or filtering")
    __properties: ClassVar[List[str]] = ["id", "type", "label", "properties"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['DomainName', 'IpAddress', 'ASN', 'Nameserver', 'MailServer', 'Network', 'Certificate']):
            raise ValueError("must be one of enum values ('DomainName', 'IpAddress', 'ASN', 'Nameserver', 'MailServer', 'Network', 'Certificate')")
        return value
