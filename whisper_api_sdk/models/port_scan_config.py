...
class PortScanConfig(BaseModel):
    ...
    """ # noqa: E501
    ports: Optional[StrictStr] = Field(default='top-1000', description="Ports to scan (comma-separated or range)")
    technique: Optional[Annotated[str, Field(strict=True)]] = Field(default='syn', description="Scan technique")
    threads: Optional[Annotated[int, Field(le=100, strict=True, ge=1)]] = Field(default=10, description="Number of parallel threads")
    port_timeout: Optional[Annotated[int, Field(le=10000, strict=True, ge=100)]] = Field(default=1000, description="Timeout per port in milliseconds")
    __properties: ClassVar[List[str]] = ["ports", "technique", "threads", "port_timeout"]
    ...
    def technique_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(syn|connect|udp|comprehensive)$", value):
            raise ValueError(r"must validate the regular expression /^(syn|connect|udp|comprehensive)$/")
        return value

    @field_validator('technique')
    def technique_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['syn', 'connect', 'udp', 'comprehensive']):
            raise ValueError("must be one of enum values ('syn', 'connect', 'udp', 'comprehensive')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "ports": obj.get("ports") if obj.get("ports") is not None else 'top-1000',
            "technique": obj.get("technique") if obj.get("technique") is not None else 'syn',
            "threads": obj.get("threads") if obj.get("threads") is not None else 10,
            "port_timeout": obj.get("port_timeout") if obj.get("port_timeout") is not None else 1000
        })
        return _obj
