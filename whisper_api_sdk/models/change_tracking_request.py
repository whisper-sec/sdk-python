...
class ChangeTrackingRequest(BaseModel):
    ...
    """ # noqa: E501
    fields: Optional[List[StrictStr]] = Field(default=None, description="Fields to track for changes. Valid values depend on indicator type.")
    frequency: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="How often to check for changes")
    enabled: Optional[StrictBool] = Field(default=None, description="Whether tracking is enabled")
    __properties: ClassVar[List[str]] = ["fields", "frequency", "enabled"]
    ...
    def frequency_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(hourly|daily|weekly)$", value):
            raise ValueError(r"must validate the regular expression /^(hourly|daily|weekly)$/")
        return value
    ...
    def frequency_validate_enum(cls, value):
        ...
        if value not in set(['hourly', 'daily', 'weekly']):
